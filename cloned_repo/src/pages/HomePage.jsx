...
const HomePage = () => {
  ...
  const { t } = useLanguage();
  const { monsters: storeMonsters, page: storePage, totalPages: storeTotalPages, setMonsters: setStoreMonsters, setPage: setStorePage, setTotalPages: setStoreTotalPages } = useMonsterStore();
  ...
  const [monsters, setMonsters] = useState(storeMonsters.length > 0 ? storeMonsters : []);
  const [totalCount, setTotalCount] = useState(0);
  const [totalPages, setTotalPages] = useState(storeTotalPages > 1 ? storeTotalPages : 1);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [currentPage, setCurrentPage] = useState(storePage > 1 ? storePage : pageParam);
  ...
  // Detect back navigation from monster page  const location = useLocation();
  ...
  // Sync state with URL params - clear if not in URL
  useEffect(() => {
    if (isBackNav.current) return; // Skip reset when coming back from monster page
    ...
    const fetchData = async () => {
      try {
        ...
        if (hasLocalFilters) {
          ...
          // Load more pages when filter is active - parallel fetch
          const pagesToFetch = filters.region ? 3 : 1;
          // Pages are 1-indexed; ensure we request the correct page numbers
          const fetchPromises = Array.from({ length: pagesToFetch }, (_, i) => 
            fetchMonsters({ page: i + 1, search: searchTerm, cr: filters.cr })
          );

          const results = await Promise.all(fetchPromises);

          // Aggregate monsters and total count from all fetched pages
          results.forEach((result, idx) => {
            const fetchedMonsters = result.monsters;
            const count = result.count;
            if (fetchedMonsters.length === 0) {
              console.log('No more results, stopping');
              return;
            }
            console.log('Page', idx + 1, 'got', fetchedMonsters.length, 'monsters');
            ...
            // Sum counts from each page to get the overall total
            totalCount += count;
          });
          ...
          setStoreTotalPages(calculatedPages);
  ...
  // Debounce filter changes to prevent premature re-fetches
  ...
  }, [extraPages, searchQuery, filters.cr]);
      ...
      {loading ? (
        ...
        <div className="monsters-grid">
          {sortedMonsters.map((m) => (
            <MonsterCard key={m.id} monster={m} currentPage={currentPage} />
          ))}
        </div>
      )}
